...
export const runPMT = async (cmd: string, options?: ExecaOptions) => {
  const args = cmd.split(' ').filter(Boolean)
  return execa('prisma-multi-tenant', args, options)
}
...
`
  if (database.multiTenant) {
    const MANAGEMENT_PROVIDER = database.multiTenant.management.provider
    const MANAGEMENT_URL = database.multiTenant.management.url
    envContent += `
